...
class VentaRepository:
    ...
    def get_hierarchical_sales_report(
        ...
        if tipo_inventario == 'actual':
            ...
            tabla_inventario_almacen = 'inventario_almacen'
            filtro_fecha = ''
            ...
            tabla_inventario_almacen = 'historial_inventario_almacen'
        ...
        # Generar las condiciones dinamicas para los filtros de almacen
        condicion_nombre_marca = [
            "marca.nombre = :nombre_marca",
        ]

        # Creamos la clausula con las condiciones
        where_clause_nombre_marca = " AND ".join(condicion_nombre_marca)

        # Creamos la clausula where sin los filtros de producto
        where_clase_without_product_conditions = " AND ".join(condiciones)
        ...
        if nombre_marca == 'AY GÜEY':
            ...
            WITH 
                ...
                PresupuestosTiendas AS (
                    ...
                    JOIN marca ON tienda.id_marca = marca.id_marca 
                    CROSS JOIN variables
                    WHERE {where_clase_without_product_conditions}
                ...
                SELECT 
                    ...
                    COALESCE(t.nombre,i.nombre, 'SIN NOMBRE') AS nombre,
            ...
            InventariosAlmacenesVirtualesPorNivel AS (
                ...
                FROM {tabla_inventario_almacen}
                JOIN producto ON producto.id_producto = {tabla_inventario_almacen}.id_producto
                JOIN almacen ON almacen.id_almacen = {tabla_inventario_almacen}.id_almacen
                JOIN marca ON marca.id_marca = almacen.id_marca
                CROSS JOIN variables
                WHERE 
                    {where_clause_nombre_marca} {filtro_fecha}
                ...
                JOIN marca ON marca.id_marca = almacen.id_marca
                CROSS JOIN variables
                WHERE {where_clause_nombre_marca} {filtro_fecha}
                    AND NOT almacen.whscode = ANY (variables.almacenes_fisicos)
                GROUP BY producto.categoria, producto.subcategoria
            ),
            AsignacionDeAlmacenesFisicosConVentaPromedio AS (
                ...
                JOIN marca ON marca.id_marca = almacen.id_marca
                CROSS JOIN variables
                WHERE  {where_clause_nombre_marca}
                    AND (tienda.whscode ILIKE '%MX%' OR tienda.whscode ILIKE '%MEX%')
            ...
            PiezasAlmacenFisicoPorTiendas AS (
                ...
                CROSS JOIN variables
                WHERE whscode_tienda = ANY(:whscodes)
                GROUP BY iafpnd.nivel,iafpnd.key
            )
            SELECT 
                COALESCE(t.nivel, i.nivel) AS nivel,
                COALESCE(t.key, i.key, iavpn.key) AS key,
                COALESCE(t.nombre, i.nombre, iavpn.nombre,'SIN NOMBRE') AS nombre,
                COALESCE(t.iva_anio_anterior, 0) AS iva_anio_anterior,
                COALESCE(t.iva_anio_actual, 0) AS iva_anio_actual,
                COALESCE(t.variacion_efectivo, 0) AS variacion_efectivo,
                COALESCE(t.variacion_porcentaje, 0) AS variacion_porcentaje,
